...
function sendRequest(name, phone, address, goods, sum) {
    let returnValue = {data : {client: '', order: {}, goods: []}};

    let countOfGoods = goods.length;

    for (let i = 0; i < countOfGoods; i += 1) {
        returnValue.data.goods.push({title: goods[i].title, count: goods[i].count});
    }

    returnValue.data.order.address = `ул. ${address.street}, дом ${address.house}, ${address.entrance} подъезд, ${address.floor} этаж, кв. ${address.flat}`;
    returnValue.data.order.sum = sum;

    returnValue.data.client = `${name} ${phone}`;

    let jsonData = JSON.stringify(returnValue);
